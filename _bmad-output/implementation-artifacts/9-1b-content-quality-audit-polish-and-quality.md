...
Status: done (2026-04-09 — Phase 1 build + 3 carve-outs + 2 Phase 2 polish passes shipped; full 3-step CQA pipeline verified end-to-end against the 5-URL trustworthiness gate; audit-report.md verified as paying-client quality; tests 442/442; ready for beta launch)
...
- [x] All Phase 1 Acceptance Criteria #1–#13 verified
- [x] All Phase 2 Acceptance Criteria #14–#18 verified _(Phase 2 polish budget 2/5 consumed; passes #1 and #2 applied; trustworthiness gate passed on first run after the MaxOutputTokens carve-out — full polish loop not exercised because the pipeline produced paying-client-quality output without it; remaining 3 passes available for post-launch iteration if needed)_
- [x] All Tasks 1–10 complete
- [x] `dotnet test AgentRun.Umbraco.slnx` is green (final: **442/442 passing**)
- [x] Phase 1 manual E2E gate (Task 8) verified by Adam against the 5-URL batch (`umbraco.com`, `wearecogworks.com`, `umbraco.com/products/umbraco-cms`, `bbc.co.uk/news`, `wikipedia.org/wiki/Content_management_system`) — instance `64dc770a0d6b4ae2aea9a3f97454aa02` ran the full 3-step pipeline end-to-end clean
- [x] Trustworthiness signoff artefact at `_bmad-output/qa-signoffs/9-1b-cqa-trustworthiness-signoff.md` exists, lists ≥3 representative real test inputs, and is signed by Adam _(signed 2026-04-09 against the 5-URL batch above)_
- [x] Phase 2 production smoke test executed on a clean fresh TestSite build with locked prompts _(instance `64dc770a0d6b4ae2aea9a3f97454aa02` is that smoke test)_
- [x] `read_file` is **not** called against any cached HTML payload during the audit pattern in any verified run — confirmed via conversation log inspection (scanner makes zero `read_file` calls; only the analyser and reporter call `read_file`, against the artifact files `scan-results.md` and `quality-scores.md`, which is the intended cross-step handoff pattern)
- [x] **Pre-Implementation Architect Review gate ticked** — Winston approved (see below)
- [x] **Post-implementation code review** — Bob ran adversarial review against Phase 1's code changes (3 layers: Blind Hunter, Edge Case Hunter, Acceptance Auditor); resulting fix pass shipped as commit `2dd1bbb` and validated via the manual E2E gate
...
- `AgentRun.Umbraco/Tools/FetchUrlTool.cs` — added `extract` parameter, structured-extraction code path, parser helpers, structured handle records. **Fix pass:** D2 manual redirect loop with per-hop SSRF re-validation; P1 dropped `truncated_during_parse` from `StructuredFetchHandle`; P2 case-insensitive meta selector; P3 script/style stripping in word count; P4 `<base href>` resolution; P5 empty-body short-circuit returns structured shape in structured mode. **Manual E2E carve-out:** default `User-Agent: AgentRun/1.0` set on every outgoing HttpRequestMessage (initial fetch + every redirect hop).
- `AgentRun.Umbraco/Engine/ToolLoop.cs` — **manual E2E carve-out:** single-shot stall recovery via synthetic user nudge. When an empty/narration stall is detected AND the workflow has a completion check AND that check fails, inject one nudge user message and give the model exactly one more turn before throwing `StallDetectedException`. Sibling to Story 9.0's completion-check-on-stall recovery.
...
| 2026-04-09 | 1.0 | **Story 9.1b → DONE.** Full 3-step CQA pipeline (scanner → analyser → reporter) verified end-to-end against the 5-URL trustworthiness gate batch on Sonnet 4.6 in instance `64dc770a0d6b4ae2aea9a3f97454aa02`. All 3 artefacts produced (`scan-results.md`, `quality-scores.md`, `audit-report.md`); spot-checked against engine logs (every numeric field matches the structured handles); inspected for editorial quality (evidence-cited findings, emergent cross-page reasoning, sound prioritisation). Adam's trustworthiness signoff at `_bmad-output/qa-signoffs/9-1b-cqa-trustworthiness-signoff.md` confirms `audit-report.md` is paying-client quality. DoD checkboxes all checked. 3 cosmetic / editorial-judgement polish items filed in `deferred-work.md` (date stamps, footer-vs-content H2 confusion, non-owned-domain finding deprioritisation) — none are blockers; Phase 2 polish budget 2/5 consumed with 3 remaining for post-launch trustworthiness iteration. **Story is approved for beta launch.** | Adam |
| 2026-04-09 | 0.7 | **Phase 1 carve-out #3 (root cause): set `MaxOutputTokens=32768` to fix CQA scanner stall (`FinishReason=length` on multi-URL batches).** Pre-commit diagnostic gate (`FinishReason` instrumentation in `ToolLoop.cs`) reclassified the stall from a workflow rhythm regression to a `MaxOutputTokens` ceiling bug. Architect (Winston) gated all three candidate fixes (rhythm restructure, prompt rewording iteration, model swap testing) on adding 5 lines of `FinishReason` logging first. The first instrumented run captured the smoking gun: `engine.empty_turn.finish_reason: length (accumulatedTextLength=0, updateCount=12)` — the model was being truncated mid-thought by the API, with Sonnet 4.6's thinking tokens consuming the entire output budget before any visible response tokens were produced. **Architect's pre-commit instrumentation requirement prevented a wrong fix from shipping** (the per-fetch write_file rhythm restructure would have appeared to work as a side-effect of dodging the cap and reintroduced the bug elsewhere later). **Fix:** hardcode `MaxOutputTokens = 32768` on `ChatOptions` at [`StepExecutor.cs:160`](../../AgentRun.Umbraco/Engine/StepExecutor.cs#L160). Sonnet 4.6 documented max output is 64k; 32k leaves comfortable headroom for thinking-token bursts plus a multi-page markdown write while still bounded enough to prevent runaway generation. Architect chose 32k over Amelia's 16k recommendation for the headroom. **Diagnostic logging promoted from temporary scaffolding to permanent structured engine telemetry event** `engine.empty_turn.finish_reason` — protects every future workflow from the same silent-truncation footgun. **Verification gate:** the same 5-URL batch (`umbraco.com`, `wearecogworks.com`, `umbraco.com/products/umbraco-cms`, `bbc.co.uk/news`, `wikipedia.org/wiki/Content_management_system`) that deterministically stalled across the previous 4 attempts **passed cleanly on the first run after the fix landed** — `fetch_url ×5 → write_file → Complete`, post-write narration `FinishReason: stop`. **Follow-ups filed in `deferred-work.md`:** (a) **Story 9.6.1** — wire `MaxOutputTokens` through `IToolLimitResolver` for per-step / per-workflow override (replaces the hardcoded literal); (b) **CQA scanner incremental writes for crash-resilience** — the rhythm restructure shelved-not-cancelled by Winston with explicit guidance, kept on file as a future durability improvement that does not depend on the stall narrative. **Reusable lesson recorded for future stories:** when you have multiple plausible hypotheses and a 5-line diagnostic can partition them, run the diagnostic before you commit to a fix — always, even (especially) when under time pressure with the appealing narrative right there. Phase 2 polish budget remains 2 of 5 consumed and is **not retroactively recovered** — passes #1 and #2 (verbatim invariant rewordings) stay applied because the rewordings are still independently correct. Full test suite green (442/442). **Phase 1 manual E2E gate (Task 8): PASSED on the 5-URL batch.** Story 9.1b can advance from `review` to `done` once Adam ratifies. | Amelia |
| 2026-04-09 | 0.6 | **Phase 1 engine carve-out: single-shot stall recovery via synthetic user nudge in `ToolLoop`.** After Phase 2 polish passes #1 and #2 both failed to fix the post-batch stall, a 1-URL diagnostic run **succeeded** while the 5-URL run kept stalling — proving the stall is batch-size-related, not a model-capability issue and not a prompt-wording issue. Sonnet 4.6 reliably loses track of the write_file step at N=5 cumulative turns. Diagnosis pivoted from prompt iteration to engine-side: Story 9.0's stall recovery handles "model finished but went silent" (completion check passes) but does NOT handle "model lost track and needs a nudge to finish" (completion check fails). This carve-out adds the missing sibling defence. **Change in `ToolLoop.RunAsync`:** when an empty/narration stall is detected AND the workflow has a completion check AND that check fails, inject a one-shot synthetic user nudge ("Continue with the next required action — your previous turn was empty and the task is not yet complete.") and give the model exactly one more turn before throwing `StallDetectedException`. Single attempt — `nudgeAttempted` flag prevents recursion. Gated on `completionCheck is not null` so workflows without a verifiable success signal preserve the old immediate-throw behaviour. After the nudge fires, the next no-tool-call assistant turn is treated as a confirmed stall regardless of `StallDetector` classification (the synthetic user message would otherwise mask the stall pattern from the classifier). The success path still wins on the post-nudge turn if the model called the recovery tool AND the completion check now passes. Generic wording — no workflow-specific tool name baked into the engine. 2 new tests in `ToolLoopTests.cs` (`Stall_Interactive_NarrationAndCompletionCheckFails_NudgesOnceThenThrows` and `Stall_Interactive_EmptyTurnAndCompletionCheckFails_NudgeRecoversWithToolCall`) covering both the recovery-failure and recovery-success paths. The previously-passing `Stall_Interactive_NarrationAndCompletionCheckFails_StillThrowsStall` test was updated in place to assert the new 3-LLM-call shape (was 2). All 442 tests green. Phase 2 polish budget consumed by passes #1 and #2 (2 of 5) is NOT recovered — the previous changes stay applied (rewording is correct independently). Phase 1 manual E2E gate (Task 8) re-run still owned by Adam. | Amelia |
| 2026-04-09 | 0.5 | **Phase 2 polish loop pass #2 (explicit task statement at top of prompt).** Pass #1 (Invariant #4 rewording to drop the read_file mention) did not fix the stall — manual E2E gate run #5 produced the same shape: all 5 URLs returned 200 OK with full structured fields, model stalled with empty turn after the 5th. Hypothesis 1 (read_file mention paralysing the model) is now ruled out. Hypothesis 2 (deeper Sonnet 4.6 post-batch stall) is confirmed. Diagnosis: the prompt is structured almost entirely as behavioural prohibitions ("don't do X"), with the actual *task* buried inside Invariant #4 and the Writing Results section. Sonnet 4.6 is anchoring on "what am I forbidden to do" instead of "what am I supposed to do" and treating "5 successful fetch_urls" as task-complete. **Pass #2 change:** added an explicit `## Your Task` section at the top of scanner.md (right after the title, before the behaviour rules) describing the task as a numbered step list and asserting `write_file` as the unambiguous task-completion trigger ("Calling `fetch_url` for every URL is necessary but **not sufficient**"). The five hard invariants are still untouched (the rewording from pass #1 stays). Net `scanner.md` line count: 117 vs pre-Phase-1 114 (+3, deliberate AC #10 trade — correctness wins over length in the polish loop). Phase 2 polish soft-cap budget consumed: 2 of 5. Full test suite green (441/441). Phase 1 manual E2E gate (Task 8) re-run still owned by Adam. | Amelia |
